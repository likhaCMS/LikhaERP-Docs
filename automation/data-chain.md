...
- **`$accountability`** — Provides details on who/what started the flow. This could include user's id, role, ip address, etc. Every data chain has a `$accountability` key.
...
Operation keys will contain the data (if any) generated by the first operation. If none, `null` will be appended. It will also be common to have nested JSON data.
